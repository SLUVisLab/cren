...
data_dir = "./datasets_pre_both"
store_dir= 'train_double_pre_both_dropout'
...
class ModelParallelResNet18(nn.Module):
    def __init__(self):
        ...
        self.fc=nn.Linear(1024,num_classes).to('cuda:0')
    def forward(self, x):
        x=x.transpose(1,0)
...
def four_crop(img):
    ...
    for j in range(4):
        crops.append(imgs[j])
    return crops
def four_and_random(img):
    Rand=torchvision.transforms.RandomCrop(256)
    return four_crop(img)+[Rand(img)]
