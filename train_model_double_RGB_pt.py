...
from pytorch_CAM import saliency
from torchvision.models.resnet import ResNet, BasicBlock
from greenestCrop import GreenestCrop
...
data_dir = "./datasets_RGB_june"
store_dir='./train_double_RGB_pt_june'
...
class ModelParallelResNet18(ResNet):
    def __init__(self, *args, **kwargs):
        super(ModelParallelResNet18, self).__init__(
            BasicBlock, [2,2,2,2], num_classes=num_classes, *args, **kwargs)
        ...
        self.fc=nn.Linear(self.fc.in_features*2,num_classes).to('cuda:0')
    def forward(self, x):
        x=x.transpose(1,0)
        x0=x[:16].transpose(1,0)
        x1=x[-1]
...
def sixteen_crop(img):
    crops=[]
    five512=torchvision.transforms.FiveCrop(512)
    five256=torchvision.transforms.FiveCrop(256)
    imgs=five512(img)
    for i in range(4):
        ims=five256(imgs[i])
        for j in range(4):
            crops.append(ims[j])
    return crops
def sixteen_and_green(img):
    return sixteen_crop(img)+[GreenestCrop(size=256,n=5,m=3,pad_if_needed=True)(img)]
...
def initialize_model(model_name, num_classes, feature_extract, use_pretrained=True):
    ...
    if model_name == "double":
        model_ft = ModelParallelResNet18()
        input_size=256
        ct=0
...
data_transforms = {
    'train': transforms.Compose([
        transforms.Resize(1024,interpolation=2),
        transforms.Lambda(lambda img: sixteen_and_green(img)),
        transforms.Lambda(lambda crops: torch.stack([transforms.Compose([transforms.ToTensor(),
                            transforms.Normalize(mean=[n / 255.
                                   for n in [129.3, 124.1, 112.4]], std=[n / 255. for n in [68.2, 65.4, 70.4]])])(crop) for crop in crops]))
